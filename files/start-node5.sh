...
./caminogo --http-host=${node5} --public-ip=${node5} --network-id=local --index-enabled=true --staking-enabled=true --http-port=9650 --staking-port=9651 --log-level=info --bootstrap-ids=NodeID-7Xhw2mDxuDS44j42TCB6U5579esbSt3Lg --bootstrap-ips=${node1}:9651 --staking-tls-cert-file=/files/certs/keys5/staker.crt --staking-tls-key-file=/files/certs/keys5/staker.key
